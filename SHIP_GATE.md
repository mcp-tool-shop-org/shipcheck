...
- [ ] `[cli|mcp|desktop]` SKIP: CLI only copies template files, no dangerous actions
- [x] `[cli|mcp|desktop]` File operations constrained to known directories (2026-02-27) — writes only to CWD
- [ ] `[mcp]` SKIP: not an MCP server (detected due to "mcp" in description)
...
- [ ] `[cli|mcp|desktop]` SKIP: template copier has no logging levels — output is deterministic
- [ ] `[mcp]` SKIP: not an MCP server
...
- [x] `[all]` Dependency scanning runs in CI (ecosystem-appropriate) (2026-02-27) — npm audit in ci.yml
- [ ] `[all]` SKIP: zero runtime dependencies — nothing to update
- [x] `[npm]` `npm pack --dry-run` includes: dist/, README.md, CHANGELOG.md, LICENSE (2026-02-27) — no dist/ (no build), but all docs included
- [x] `[npm]` `engines.node` set · `[pypi]` `python_requires` set (2026-02-27) — >=18.0.0
